...
async function runTests() {
  ...
  try {
    ...
    try {
      // Look for create group button - try multiple selectors
      const createBtn = page.locator('button[type="submit"], button:has-text("Crear"), a:has-text("Crear"), button:has-text("Nuevo"), a:has-text("Nuevo")').first();
      await createBtn.waitFor({ state: 'visible', timeout: 10000 });
      await createBtn.click();
      
      await page.waitForTimeout(1000);
      await takeScreenshot(page, '02-create-group-form');
      
      // Fill group form
      await page.fill('input[type="text"]', TEST_GROUP_NAME, { timeout: 5000 });
      
      // Try to find and fill slug
      const inputs = await page.locator('input[type="text"]').all();
      if (inputs.length > 1) {
        await inputs[1].fill(TEST_GROUP_SLUG);
      }
      ...
      await takeScreenshot(page, '03-group-form-filled');
      ...
      const screenshot = await takeScreenshot(page, '04-group-created');
      results.push({ 
    ...
    for (let i = 0; i < TEST_PLAYERS.length; i++) {
      try {
        ...
        const addBtn = page.locator('button:has-text("Agregar"), button:has-text("Nuevo"), a:has-text("Agregar")').first();
        await addBtn.click();
