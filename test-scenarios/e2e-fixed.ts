...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      throw error; // Stop if we can't even load the page
    }

    // Test 2: Create group
    console.log('Test 2: Creating group...');
    try {
      // Fill group name - first text input
      await page.fill('input[placeholder*="Nombre"], input[type="text"]:nth-of-type(1)', TEST_GROUP_NAME, { timeout: 10000 });
      
      // Fill passphrase - second text input
      await page.fill('input[placeholder*="Clave"], input[type="text"]:nth-of-type(2)', TEST_PASSPHRASE, { timeout: 10000 });
    ...
    for (let i = 0; i < TEST_PLAYERS.length; i++) {
      try {
        ...
        await page.click('button:has-text("Agregar"), button:has-text("Nuevo"), a:has-text("Agregar jugador")');
