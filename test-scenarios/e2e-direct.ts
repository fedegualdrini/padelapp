...
const TEST_URL = 'https://padelapp-oj93bi5g4-fedegualdrini-gmailcoms-projects.vercel.app';
const TEST_GROUP_NAME = `E2E Test ${Date.now().toString().slice(-4)}`;
const TEST_GROUP_SLUG = `e2e-${Date.now().toString().slice(-6)}`;
const TEST_PASSPHRASE = `pass-${randomUUID().slice(0, 6)}`;
...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      const createBtn = page.locator('button, a').filter({ hasText: /crear|Crear|nuevo|Nuevo/i }).first();
      await createBtn.click({ timeout: 10000 });
      
      // Fill form
      await page.fill('input[type="text"], input[name*="name"]', TEST_GROUP_NAME, { timeout: 5000 });
      await page.fill('input[name*="slug"], input:nth-of-type(2)', TEST_GROUP_SLUG, { timeout: 5000 });
      
      // Look for passphrase field
      const passphraseInput = page.locator('input[type="password"], input[name*="pass"], input[placeholder*="contraseña"]').first();
      if (await passphraseInput.isVisible().catch(() => false)) {
        await passphraseInput.fill(TEST_PASSPHRASE);
      }
      
      await takeScreenshot(page, '02-group-form');
      ...
      results.push({ 
        ...
        status: 'PASS', 
        screenshot,
        notes: `Group: ${TEST_GROUP_NAME}, slug: ${TEST_GROUP_SLUG}`
      });
    ...
    for (let i = 0; i < TEST_PLAYERS.length; i++) {
      try {
        await page.goto(`${TEST_URL}/g/${TEST_GROUP_SLUG}/players`, { waitUntil: 'networkidle' });
        await takeScreenshot(page, `04-players-page-${i+1}`);
        
        // Click add player
        const addBtn = page.locator('button, a').filter({ hasText: /agregar|Agregar|nuevo|Añadir/i }).first();
        await addBtn.click();
        
        // Fill player name
        await page.fill('input[type="text"], input[name*="name"]', TEST_PLAYERS[i]);
    ...
    try {
      await page.goto(`${TEST_URL}/g/${TEST_GROUP_SLUG}/matches/new`, { waitUntil: 'networkidle' });
      ...
      if (selects.length >= 4) {
        for (let i = 0; i < 4; i++) {
          ...
          if (options.length > 1) {
            await selects[i].selectOption({ index: Math.min(i + 1, options.length - 1) });
          }
        }
      }
      
      // Fill scores if inputs exist
