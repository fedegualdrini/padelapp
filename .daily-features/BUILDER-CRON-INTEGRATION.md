# Builder E2E Test Instructions for Cron Update

When updating the builder cron job, integrate these instructions into the `payload.message` field. Paste this section BEFORE the "PRE-CHECK" section.
...
To integrate this into the builder cron job:

1. **Copy** the section above (from "CRITICAL:" to end)
2. **Paste** into the builder's `payload.message` field
3. **Position**: BEFORE the "PRE-CHECK:" line
4. **Run**: `gateway restart` to apply changes

## Current Builder Job ID

`5dad283e-5c3f-44c3-afb4-9234078274f8`

## Important Reminder

**After updating any cron job configuration, ALWAYS run `gateway restart` for changes to take effect!**

(See LEARNINGS.md - LRN-20260130-001)
